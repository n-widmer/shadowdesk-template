...
if [[ "$FOREGROUND" == "true" ]]; then
  echo "$$" > "$PID_FILE"
  # exec so node inherits THIS pid ($$), which is what we just wrote to PID_FILE.
  # Without exec, node is a child with a different pid and stop-server kills the wrapper only.
