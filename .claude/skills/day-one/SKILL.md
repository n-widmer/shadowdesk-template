...
Parse the `APIFY_TOKEN=` line from inside the `SHADOWDESK_BUNDLE v1` HTML-comment block. Hold the value in working memory for this session only. Do **not** `setx` it, do **not** write it to `.env`, do **not** commit it. The token vanishes when this session ends — that's intentional.
